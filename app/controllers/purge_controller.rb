class PurgeController < ApplicationController
  ...
  def purge_master_tables
    ...
    if params[:to_date].to_date >=params[:from_date].to_date
      begin
        ...
        #----------------------------------------------Counter Collection ----------------------------------------------------------
        counter_collection_max_date = Countercollection.maximum(:date)
        count_defore_countercollection = Countercollection.find(:all,:conditions => ["date!=? and date>=? and date<=?",counter_collection_max_date,params[:from_date],params[:to_date]]).count
        count_after_countercollection = Countercollection.delete_all(["date!=? and date>=? and date<=?",counter_collection_max_date,params[:from_date],params[:to_date]])
  ...
  # AS In Site application provided purging option for tables ,following listed tables
  # had option for purging.
  # For first 2 listed below had auto purging option and last 3 provided UI for purge.

  # 1. Counter collection - auto_purge
  # 2. Client data - auto_purge

  # 3. Counter collection - manual purge
